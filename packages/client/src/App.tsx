...
export function App() {
  ...
  const { roomInfo, gameState, events, consumeSkipAnimations } = useGameState(on, emit);
  ...
  // Keep state in sync with URL (browser back/forward)
  useEffect(() => {
  ...
  useEffect(() => {
    const unsub1 = on('reconnected', (data: { playerId: string; roomId: string; token: string }) => {
      setPlayerId(data.playerId);
      setRoomId(data.roomId);
      setToken(data.token);
      // Ensure URL has the token
      const url = `/room/${data.roomId}?t=${data.token}`;
      window.history.replaceState(null, '', url);
      saveSession(data.token, data.roomId);
    });
  ...
  }, [on, saveSession, clearSession]);

  const navigateToRoom = useCallback((rid: string, t: string) => {
    window.history.pushState(null, '', `/room/${rid}?t=${t}`);
    setRoomId(rid);
    setToken(t);
  }, []);
  ...
  const handleCreateRoom = (playerName: string) => {
    emit('create_room', { playerName }, (res: any) => {
      ...
      navigateToRoom(res.roomId, res.token);
    });
  };
  ...
  // URL has a room but no token — need to join via lobby
  if (roomId && !token) {
    return <Lobby onCreateRoom={handleCreateRoom} onJoinRoom={handleJoinRoom} prefillRoomId={roomId} />;
  }

  // URL has room + token but server hasn't confirmed yet — waiting for reconnect
  if (roomId && token && !playerId) {
    return (
      <div className="page" style={{ textAlign: 'center', paddingTop: '100px' }}>
        <h2>正在恢复游戏...</h2>
        <p style={{ color: '#888', marginTop: '12px' }}>如果长时间无响应，请刷新页面</p>
      </div>
    );
  }
  ...
  return (
    <Race
      ...
      events={events}
      onAction={handleGameAction}
      consumeSkipAnimations={consumeSkipAnimations}
    />
  );
}
